# -*- coding: utf-8 -*-
"""Rank_exp_on_GPT2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/19Pncp4VJValhf4BS1ngsC4_7He919tyT
...
"""Define LoRA modules for GPT-2 Conv1D layers

GPT-2 stores attention as two Conv1D layers per block:

- `attn.c_attn`: shape `(d_model, 3*d_model)` -- a *fused* Q/K/V projection.
- `attn.c_proj`: shape `(d_model, d_model)` -- the output projection (W_O).

To match the RoBERTa-style rank sweep we need to be able to apply LoRA
to **individual slices** of `c_attn` (just Q, just K, just V, or any
combination), and to apply a separate LoRA to `c_proj` for the O target.
`LoRAConv1DAttn` handles the sliced case; `LoRAConv1D` is the plain
wrapper used for `c_proj`.
"""

class LoRAConv1D(nn.Module):
    """Plain LoRA wrapper for a GPT-2 Conv1D (used here for c_proj / W_O).

    Conv1D weight shape is (in_features, out_features), and the forward
    is `out = x @ W + b` (note: no transpose), so LoRA factors A, B are
    shaped to match: A is (in, r), B is (r, out).
    """
...
class LoRAConv1DAttn(nn.Module):
    """LoRA wrapper for GPT-2's fused c_attn Conv1D (Q/K/V combined).

    For each requested slice (`q`, `k`, `v`) we keep an independent rank-`r`
    LoRA adapter and add its delta only to the matching slice of the
    `(..., 3*d_model)` output. This makes Q-only / K-only / V-only / Q+K /
    Q+V / Q+K+V LoRA cleanly addressable on GPT-2 the same way Q/K/V are
    independently addressable on RoBERTa's three separate Linear layers.
    """
...
def apply_lora_to_attention(
    ...
    """Apply LoRA to selected attention projections in a GPT-2 model.

    `targets` is any subset of {"q", "k", "v", "o"}:
        q -> slice 0:d_model    of attn.c_attn
        k -> slice d_model:2*d  of attn.c_attn
        v -> slice 2*d:3*d      of attn.c_attn
        o -> attn.c_proj        (W_O, output projection)
    """
    valid = {"q", "k", "v", "o"}
    targets = tuple(t.lower() for t in targets)
...
# Demo: apply default Q+V LoRA at r=8 to the loaded `model` so the cells
...
# matching the GPT-2 cell in LoRA_project.ipynb. We shorten the epochs
...
Mirrors the GPT-2 evaluation in `LoRA_project.ipynb` (cell 59): generate
with beam search from `"{mr} => "` prompts, group multiple human
references per MR, and score with sacrebleu's corpus-level multi-reference
BLEU.
"""
...
def evaluate_gpt2_bleu(
    ...
    # sacrebleu wants references as parallel streams: ref_streams[k][i]
    # is the k-th reference for the i-th prediction (or None if missing).
    bleu = BLEU()
...
All settings share the **same trainable LoRA parameter budget**
(rank x num_matrices held constant: 8x1 = 8, 4x2 = 8, 2x4 = 8) so BLEU
differences reflect *where* parameters are spent, not *how many*.
...
| 3 | V only        | `c_attn[V]`                     | 8    |
| 4 | O only        | `c_proj`                        | 8    |
| 5 | Q + K         | `c_attn[Q]` + `c_attn[K]`       | 4    |
| 6 | Q + V         | `c_attn[Q]` + `c_attn[V]`       | 4    |
| 7 | Q + K + V + O | `c_attn[Q,K,V]` + `c_proj`      | 2    |

Optimizer hyperparameters reuse the GPT-2 / E2E setup from
`LoRA_project.ipynb`: `batch=8`, `lr=2e-4`, `AdamW(weight_decay=0.01)`,
`warmup_steps=500`, `linear` schedule, `alpha=8`, `max_seq_len=128`.
Evaluation uses the exact multi-reference sacrebleu function from that
notebook (`evaluate_gpt2_bleu`). Results for each setting are written
under `results/` with the LoRA targets and rank in the filename.
"""

# ------------------------------------------------------------------
# Rank Sensitivity Sweep on GPT-2 / E2E NLG
#
# Each setting trains a *fresh* GPT-2 from `model_name`, applies LoRA only
# to the listed attention projections at the specified rank, then trains
# the LoRA params with the paper's GPT-2 / E2E schedule.
#
...
LORA_DROPOUT = 0.1    # matches GPT-2 LoRA setting in LoRA_project.ipynb
...
"""## Bar plot of final E2E BLEU per rank-sweep setting

Reads `rank_sweep_summary` (built by the sweep cell above). If the
variable isn't in scope (e.g. you re-opened the notebook), it rebuilds
the summary from the JSON files saved under `results/` and keeps the
most recent result per LoRA-target combination. The figure is also saved
under `results/` for later use.
"""
